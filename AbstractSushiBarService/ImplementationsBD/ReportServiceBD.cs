    ...
    public class ReportServiceBD : IReportService
    {
        ...
        public void SaveSushiPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var sushis = context.Sushis.ToList();
                //создаем таблицу
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, sushis.Count, 2, ref missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = sushis[i].SushiName;
                    table.Cell(i + 1, 2).Range.Text = sushis[i].Price.ToString();
                }
        ...
        public List<VisitorZakazsModel> GetVisitorZakazs(ReportBindingModel model)
        {
            return context.Zakazs
                            .Include(rec => rec.Visitor)
                            .Include(rec => rec.Sushi)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new VisitorZakazsModel
                            {
        ...
        public void SaveVisitorZakazs(ReportBindingModel model)
        {
            ...
            table.AddCell(new PdfPCell(new Phrase("Суши", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            //заполняем таблицу
            var list = GetVisitorZakazs(model);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].SushiName, fontForCells));
                table.AddCell(cell);
