...
{
    public static class MailVisitor
    {
        private static TcpClient mailVisitor;
        ...
        public static void Connect()
        {
            ...
            mailVisitor = new TcpClient();
            mailVisitor.Connect("pop.gmail.com", 995);

            stream = new SslStream(mailVisitor.GetStream());
        ...
        private static void GetMessages(int number)
        {
            ...
            string zakazSubjectMessage = string.Empty;
            string zakazBodyMessage = string.Empty;
            ...
            while (true)
            {
                ...
                if (response.Length > 4)
                {
                    ...
                    {
                        zakazSubjectMessage = GetSubject(ref response, ref coding);

                        zakazBodyMessage = GetBody(response, coding);
                    }
                    if (!string.IsNullOrEmpty(messageId) && !string.IsNullOrEmpty(from) &&
                        !string.IsNullOrEmpty(zakazSubjectMessage) && !string.IsNullOrEmpty(date))
                    {
                        APIClient.PostRequestData("api/MessageInfo/AddElement",
                            new MessageInfoBindingModel
                            {
                                ...
                                Subject = zakazSubjectMessage,
                                Body = zakazBodyMessage
                            });

                        messageId = string.Empty;
                        from = string.Empty;
                        date = string.Empty;
                        zakazSubjectMessage = string.Empty;
                        zakazBodyMessage = string.Empty;
                    }
                }
            }
        }
